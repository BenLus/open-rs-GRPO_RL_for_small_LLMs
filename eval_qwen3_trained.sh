...
# Base model configuration - using local model path
MODEL_BASE_DIR="./data/OpenRS-GRPO_Qwen3"
BASE_MODEL_ARGS="dtype=bfloat16,max_model_length=32768,gpu_memory_utilization=0.6,generation_parameters={max_new_tokens:32768,temperature:0.6,top_p:0.95}"
...
run_evaluation() {
    ...
    output_dir="logs/evals/Exp${experiment}_${step}_Qwen3_trained"
    ...
    # Use local checkpoint directory for this step
    model_path="$MODEL_BASE_DIR/checkpoint-$step"
    
    # Check if checkpoint exists
    if [ ! -d "$model_path" ]; then
        echo "Error: Checkpoint directory $model_path does not exist"
        return 1
    fi
    
    # Set model args with the local path
